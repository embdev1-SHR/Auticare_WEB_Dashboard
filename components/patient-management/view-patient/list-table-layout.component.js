...
import SimpleBarComponent from "../../shared/simplebar";

const ListTableLayout = (props) => {
  ...
  const {
    ...
    setPageSize,
    rows,
    state: { pageIndex, pageSize = 10, globalFilter },
    setGlobalFilter,
  } = tableInstance;
  ...
  useEffect(() => {
    //  dispatch(setExportData({ headerMergeData, filteredData }));
    dispatch(setExportData(headerMergeData));
  }, [dispatch, headerMergeData]);
                  ...
                  {headerGroups.map((headerGroup, i) => (
                    <tr key={i} {...headerGroup.getHeaderGroupProps()}>
                      {headerGroup.headers.map((column, k) => (
                        <th key={k} className={column.isSorted ? (column.isSortedDesc ? "sorting_desc" : "sorting_asc") : "sorting"} {...column.getHeaderProps(column.getSortByToggleProps())}>
