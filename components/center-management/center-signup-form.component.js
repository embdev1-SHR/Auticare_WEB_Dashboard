...
const signupSchema = yup.object().shape({
  CenterName: yup.string().min(2, "Too Short!").max(200, "Too Long!").required("Center name is required"),
  ...
  const onSubmit = async (values) => {
    ...
    try {
      const res = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/auth/center-signup`,
        {
      ...
      {({ errors, touched }) => (
        <Form>
          <FormGroup className="mb-3">
            <Label htmlFor="CenterName">Center Name <span className="text-danger">*</span></Label>
            <Field type="text" name="CenterName" id="CenterName" className="form-control" placeholder="Enter center name" />
...
export default CenterSignupForm;
