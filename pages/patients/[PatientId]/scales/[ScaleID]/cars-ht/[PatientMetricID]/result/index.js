...
const ScreeningResult = () => {
  ...
  useEffect(() => {
    dispatch(fetchMetricResponseDetails({ PatientID: PatientId, PatientMetricID: PatientMetricID }));
    dispatch(fetchScreeningMetricResponseList({ PatientID: PatientId, PatientMetricID: PatientMetricID }));
  }, [PatientId, PatientMetricID, ScaleID]);

  useEffect(() => {
    const unique = [...new Set(responseList?.map((item) => item.ResponseSelected))];
    setOptions(unique);
    const series = unique?.map((response) => {
      const count = responseList.filter((obj) => obj.ResponseSelected === response).length;
      return count;
    });
    setSeries(series);
