...
function TrivandrumScale() {
  ...
  const arrangeQuestions = (questions, catID) => {
    return questions.filter((q) => q.CategoryID === catID).sort((a, b) => a.QuestionNumber - b.QuestionNumber);
  };
  ...
  useEffect(() => {
    ...
    dispatch(changeTitle("Trivandrum Scale"));
  ...
  const onHandleConfirm = () => {
    const ValueToSend = { ScaleID, id };
    const ValueToSendMetric = { ScaleID, MetricId, ScaleMetricType: "Screening" };
        ...
        {Loading ? (
          ...
          >
            {({ touched, errors, handleSubmit, resetForm, setFieldValue, isSubmitting }) => (
              <Row>
                <Col md={6}>
                  {UserData.UserID === scaleDetails?.Create_By && (
                    <div className="mb-4">
                      ...
                      <Row>
                        <Col md={10}>
                          <Field
                            ...
                            id="category-name"
                            placeholder="Enter domain (e.g. Gross Motor, Language)"
                          />
                        ...
                        {values.map((category, key) => (
                          <NavItem key={key}>
                            <NavLink
                              ...
                              onClick={() => toggleTab(key + 1, category.CategoryID)}
                            >
                              <CategoryItem
                                index={key + 1}
                                Category={category}
...
export default withAuth(TrivandrumScale, "ScaleManagement");
